...
def link_service_to_org(admin_client, service, organisation):
    resp = admin_client.post(
        f"/organizations/{organisation.id}/service",
...
def list_org_services(admin_client, organisation):
    resp = admin_client.get(f"/organizations/{organisation.id}/services")
    return {"status_code": resp.status_code, "json": resp.json}
...
def get_services_with_usage(admin_client, organisation):
    resp = admin_client.get(f"/organizations/{organisation.id}/services-with-usage?year=2024")
    return {"status_code": resp.status_code, "json": resp.json}
...
def response_has_n_services(api_response, count):
    ...
    # Response may be a bare list or wrapped in {"services": [...]}
